import { ap as NOOP, aq as extend$1, ar as isString, as as NO, at as isSymbol, au as isBuiltInDirective, av as capitalize, aw as camelize, ax as EMPTY_OBJ, ay as isObject, az as toHandlerKey, aA as isArray, aB as isOn, aC as isReservedProp, aD as isHTMLTag, aE as isSVGTag, aF as isMathMLTag, aG as isVoidTag, aH as parseStringStyle, aI as makeMap, aJ as generateCodeFrame, aK as getAugmentedNamespace, aL as runtimeDom_esmBundler, aM as shared_esmBundler, aN as getDefaultExportFromCjs, _ as _export_sfc, s as stores, r as reactive, x as ref, j as computed, t as resolveComponent, c as createElementBlock, f as openBlock, d as createCommentVNode, l as createBaseVNode, z as normalizeStyle, n as normalizeClass, F as Fragment, m as renderList, b as createBlock, v as toDisplayString, y as renderSlot, u as useI18n, o as onMounted, a as onBeforeUnmount, aO as createSlots, k as withCtx, h as unref, e as createVNode, X as TrackPreview, B as Button, C as withModifiers, I as IconButton, K as BaseModal } from './index-CmkoNwy1.js';
/* empty css                                                                         */
import { E as EmptyLabel } from './EmptyLabel-CuidP0V8.js';
...
const VERTICAL_LOCK_PX = 10;
    const HORIZONTAL_ACTIVATE_PX = 10;
    const ACTION_TRIGGER_PX = 60;
    const LI_WIDTH = 80;
...
const _sfc_main$1 = {
  ...
  setup(__props, { emit: __emit }) {
    ...
    const maxOffset = computed(() => props.buttons.length * LI_WIDTH + parseInt(LI_WIDTH / 2)); // ширина кнопок

    const contentStyle = computed(() => {
        const x = data.deltaX;
        return {
            transform: `translateX(${x}px)`,
            transition: data.dragging ? '' : 'transform 0.25s ease',
            touchAction: Math.abs(data.deltaX) > HORIZONTAL_ACTIVATE_PX
                ? 'none'
                : 'pan-y'
        };
    });
    ...
    const onTouchMove = (e) => {
        if (!data.dragging || data.locked) {
            return;
        }

        const t = e.touches[0];
        data.deltaX = t.clientX - data.startX;
        data.deltaY = t.clientY - data.startY;

        // если пользователь начал вертикальный скролл
        if (Math.abs(data.deltaY) > VERTICAL_LOCK_PX &&
            Math.abs(data.deltaY) > Math.abs(data.deltaX)
        ) {
            data.locked = true;
            reset();
            return;
        ...
        effectiveX = Math.min(
            Math.max(effectiveX, -maxOffset.value),
            maxOffset.value,
            0
        );

        data.deltaX = locale.rtl ? -effectiveX : effectiveX;
    };
    ...
    const button = computed(() => {
        ...
        if (absX >= ACTION_TRIGGER_PX && props.buttons.length) {
            const index = Math.min(
                Math.round(absX / LI_WIDTH),
                props.buttons.length
            );

            return props.buttons[index - 1];
        }
    });
...
return (_ctx, _cache) => {
  ...
  return (openBlock(), createElementBlock("div", {
    class: "swipe-wrapper lock-page-swipe",
    onTouchstartPassive: onTouchStart,
    onTouchmove: onTouchMove,
    onTouchend: onTouchEnd,
    onTouchcancel: onTouchEnd
  }, [
    (props.buttons.length && data.deltaX)
      ? (openBlock(), createElementBlock("ul", {
          key: 0,
          class: normalizeClass(["actions", { active: button.value?.code }]),
          style: normalizeStyle({
                'background-color': button.value?.color ? button.value.color : 'transparent',
                width: Math.abs(data.deltaX) + 'px'
            })
        }, [
...
const SwipeAction = /*#__PURE__*/_export_sfc(_sfc_main$1, [['__scopeId',"data-v-d969c254"]]);
 ...
 * License: MIT license
 */
var shake = shake$1.exports;

var hasRequiredShake;

function requireShake () {
	if (hasRequiredShake) return shake$1.exports;
	hasRequiredShake = 1;
	(function (module) {
		...
		} (typeof window !== 'undefined' ? window : shake, function (window, document) {
	...
	return shake$1.exports;
}

var shakeExports = requireShake();
const Shake = /*@__PURE__*/getDefaultExportFromCjs(shakeExports);

const _hoisted_1 = { class: "track-box" };
...
const refreshSlider = async () => {
    ...
    //     const clone = player.panes.default?._item;
    //     delete player.panes.default._item;

    //     await nextTick();

    //     player.panes.default._item = clone;
    // }
};
...
const itemAction = (item, action) => {
    ...
    if (action === 'delete') {
        setTimeout(() => {
            player.queue.removeTrack(item.id);
        }, 100);
    }
};
...
const requestMotionPermission = async () => {
    if (
        ...
        if (permission !== 'granted') {
            alert('Нет доступа к датчику движения');
            return false
        }
    }
    return true
...
const startShake = async () => {
    ...
    data.shakeEvent = new Shake({
        threshold: 15, // чувствительность
        // timeout: 1000
    });

    data.shakeEvent.start();
...
onBeforeUnmount(() => {
    if (data.shakeEvent) {
        data.shakeEvent.stop();
        window.removeEventListener('shake', shuffle);
    }
});
  ...
  return (openBlock(), createBlock(BaseModal, {
    ...
    (data.ready)
      ? {
          name: "content",
          fn: withCtx(() => [
            (unref(player).panes.default?._queue?.items?.length)
              ? (openBlock(), createBlock(unref(draggable), {
                  key: 0,
                  modelValue: unref(player).panes.default._queue.items,
                  "onUpdate:modelValue": _cache[2] || (_cache[2] = $event => ((unref(player).panes.default._queue.items) = $event)),
                  ...
                  item: withCtx(({ element: item }) => [
                    createBaseVNode("li", {
                      ...
                      createVNode(SwipeAction, {
                        buttons: [
                                {
                                    code: 'delete',
                                    name: unref(t)('modals.tracks.actions.remove'),
                                    icon: 'close',
                                    color: 'var(--color--option-5)'
                                },
                                {
                                    code: 'track-info',
                                    name: unref(t)('modals.tracks.actions.about'),
                                    icon: 'help',
                                    color: 'var(--color--option-2)'
                                }
                            ],
                        onChange: $event => (itemAction(item, $event))
                      }, {
                        default: withCtx(() => [
              ...
              : (openBlock(), createBlock(EmptyLabel, {
                  key: 1,
                  class: "empty",
                  title: unref(t)(`modals.tracks.empty.title`),
                  description: unref(t)(`modals.tracks.empty.description`)
...
const ModalTracks = /*#__PURE__*/_export_sfc(_sfc_main, [['__scopeId',"data-v-3307e4ab"]]);

export { ModalTracks as default };
//# sourceMappingURL=ModalTracks-D4HHLvPb.js.map
